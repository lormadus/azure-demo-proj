resource "azurerm_virtual_machine_scale_set" "user50vmss" {
name = "user50vmss"
...
ssh_keys {
    path = "/home/myadmin/.ssh/authorized_keys"   ## pwd 실행후 경로설정 ex) /home/user50 등 
...
network_profile {
        ...
        primary = true
        subnet_id = azurerm_subnet.user50-subnet1.id
        load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.user50-bpepool.id]
        load_balancer_inbound_nat_rules_ids = [azurerm_lb_nat_pool.lbnatpool.id]
    }
        network_security_group_id = azurerm_network_security_group.user50nsg.id
}
tags = {
    environment = "staging"
    }
